...
import { getFinanceArticle, getAllFinanceSlugs, getFinanceArticles } from '@/lib/content'
...
export async function generateStaticParams() {
  const slugs = getAllFinanceSlugs()
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const article = getFinanceArticle(params.slug)
  ...
  const articleUrl = getArticleUrl(params.slug, 'finance')
  const siteUrl = getSiteUrl()
  
  return {
    title: `${article.title} | தமிழ் நிதி கல்வி`,
    description: article.description || article.excerpt,
    keywords: ['நிதி கல்வி', 'Personal Finance Tamil', 'Tamil Finance', article.title],
    openGraph: {
      title: article.title,
      description: article.description || article.excerpt,
      type: 'article',
...
export default function FinanceArticlePage({ params }: PageProps) {
  const article = getFinanceArticle(params.slug)
  ...
  const allFinanceArticles = getFinanceArticles()
  const relatedArticles = allFinanceArticles
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      ...
      <nav className="mb-6 text-sm text-gray-600">
        ...
        <Link href="/finance" className="hover:text-blue-600">நிதி கல்வி</Link>
        <span className="mx-2">/</span>
        ...
        <section className="mt-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {relatedArticles.map((related) => (
              ...
              >
                <Link href={`/finance/${related.slug}`}>
                  <h3 className="text-lg font-semibold text-gray-900 mb-2 hover:text-blue-600 line-clamp-2 tamil-text" style={{ wordBreak: 'keep-all', overflowWrap: 'normal', lineHeight: '1.8' }}>
                    {related.title}
                  </h3>
      ...
      <div className="mt-8 text-center">
        ...
        >
          ← நிதி கல்வி பக்கத்திற்கு திரும்பு
        </Link>
      </div>
    </div>
  )
